...
import 'package:flutter_redux_boilerplate/core/widgets/table_calendar/calendar.dart';


const double dxMax = 1.2;
const double dxMin = -1.2;
...
class TableCalendarController {
  ...
  List<DateTime> get _visibleDays =>
      calendarFormat == CalendarFormat.month && !_includeInvisibleDays
          ? visibleDays.value.where((day) => !isExtraDay(day)).toList()
          : visibleDays.value;
  ...
  ValueNotifier<List<DateTime>> visibleDays;
  ...
  void init({
    ...
    visibleDays = ValueNotifier(_getVisibleDays());
    _previousFirstDay = visibleDays.value.first;
    _previousLastDay = visibleDays.value.last;

    _calendarFormat.addListener(() {
      visibleDays.value = _getVisibleDays();
    });

    if (onVisibleDaysChanged != null) {
      visibleDays.addListener(() {
        if (!_isSameDay(visibleDays.value.first, _previousFirstDay) ||
            !_isSameDay(visibleDays.value.last, _previousLastDay)) {
          _previousFirstDay = visibleDays.value.first;
          _previousLastDay = visibleDays.value.last;
  ...
  void selectPreviousTwoWeeks() {
    if (visibleDays.value.take(7).contains(_focusedDay)) {
      // in top row
  ...
  void selectNextTwoWeeks() {
    if (!visibleDays.value.skip(7).contains(_focusedDay)) {
  ...
  DateTime _getFirstDay({@required bool includeInvisible}) {
    if (_calendarFormat.value == CalendarFormat.month && !includeInvisible) {
      ...
      return visibleDays.value.first;
    }
  }
